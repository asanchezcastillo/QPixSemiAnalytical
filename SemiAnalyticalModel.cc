...
double SemiAnalyticalModel::VUVVisibility( Point_t const& ScintPoint, OpticalDetector const& opDet) const
{
 ...
 else
 {
  ...
  double r = std::hypot(ScintPoint.y - fanode_centre.y, ScintPoint.z - fanode_centre.z);
  ...
  //s1 = interpolate(fborder_corr_angle, fborder_corr_flat[0], theta, true);
  //s2 = interpolate(fborder_corr_angle, fborder_corr_flat[1], theta, true);
  //s3 = interpolate(fborder_corr_angle, fborder_corr_flat[2], theta, true);
                                      ...
                                      const int NumPhotons) const
{
  for (size_t i = 0; i < OpDetVisibilities.size(); ++i) {
    DetectedNumPhotons[i] = OpDetVisibilities[i] * NumPhotons;
  }
}
...
void SemiAnalyticalModel::CheckOverlaps() const
{
  ...
  if(!fDetectorPlaneXZ)
  {
   for(size_t i = 0; i<fOpDetCenter.size(); i++)
   {
    ...
    {
      if( ( (fOpDetCenter.at(i).z-(fOpDetLength.at(i)/2))<(fOpDetCenter.at(j).z+(fOpDetLength.at(j)/2)) 
      && (fOpDetCenter.at(j).z-(fOpDetLength.at(j)/2))<(fOpDetCenter.at(i).z+(fOpDetLength.at(i)/2))) &&
      ( (fOpDetCenter.at(i).y-(fOpDetHeight.at(i)/2))<(fOpDetCenter.at(j).y+(fOpDetHeight.at(j)/2)) 
      && (fOpDetCenter.at(j).y-(fOpDetHeight.at(j)/2))<(fOpDetCenter.at(i).y+(fOpDetHeight.at(i)/2)) )
      )
      {
      std::cout << "[ " << (fOpDetCenter.at(i).z-(fOpDetLength.at(i)/2)) << " " << (fOpDetCenter.at(j).z+(fOpDetLength.at(j)/2)) << " ], [ " << (fOpDetCenter.at(i).x-(fOpDetHeight.at(i)/2)) << " " <<(fOpDetCenter.at(i).z+(fOpDetLength.at(i)/2)) <<" ]" << std::endl;
  ...
  else
  {
   for(size_t i = 0; i<fOpDetCenter.size(); i++)
   {
    for(size_t j=i+1; j<fOpDetCenter.size(); j++)
    {
      if( ( (fOpDetCenter.at(i).z-(fOpDetLength.at(i)/2))<(fOpDetCenter.at(j).z+(fOpDetLength.at(j)/2)) 
      && (fOpDetCenter.at(j).z-(fOpDetLength.at(j)/2))<(fOpDetCenter.at(i).z+(fOpDetLength.at(i)/2))) &&
      ( (fOpDetCenter.at(i).x-(fOpDetHeight.at(i)/2))<(fOpDetCenter.at(j).x+(fOpDetHeight.at(j)/2)) 
